...
////		System.out.println(queue.size());
		while(!queue.isEmpty()) {
			...
			int num = now.num; int time = now.time;
//			System.out.println("num:"+num);
			//걸린시간은 이전까지 걸린 시간과 해당 건물을 짓는 시간을 더한 값
			if(visit[num]) continue;
			visit[num] =true;
			lapsed_times[num]=Math.max(time+times[num],lapsed_times[num]);
			
			for(int i=0;i<graph[num].size();i++) {
				int to = (int) graph[num].get(i);
				degree[to]--;
				if(!visit[to]&&degree[to]==0) {
					queue.offer(new building(to, lapsed_times[num]));
