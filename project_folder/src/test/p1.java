 ...
 * @category @problem_description 0. 받았던던 것부터 차례대로 격자에 맞춰서 붙이려고 한다. 1. 스티커를
 *           회전시키지 않고 모눈종이에서 떼어낸다. 2. 다른 스티커와 겹치거나 노트북을 벗어나지 않으면서 스티커를 붙일 수 있는
 *           위치를 찾는다. 위쪽부터 스티커를 3. 선택한 위치에 스티커를
 * @solving_description
 */
...
public class p1 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int tc = 0; tc < k; tc++) { // k개를 전부 맞춰본다
			...
			int row = rc_list.get(tc).row;
			int col = rc_list.get(tc).col;
	...
	static void rotate() {
		...
		for (int i = 0; i < c; i++)
			for (int j = 0; j < r; j++)
				map[i][j] = tmp[r - 1 - j][i]; //행의 크기(r)-1에서 회전 전의 행을 뺀 값과 같다.
