...
//			System.out.println(y+","+x);
			int dir =now.dir;//왼쪽으로 틀어준다.
			
			//2. 현재방향을 기준으로 왼쪽방향부터 차례대로 탐색
			// a. 왼쪽 방향 청소안되있으면 한칸 전진 후 1번부터 다시 진행  -> 전진 후 큐에 넣어준다.
			// b. 왼쪽 방향에 청소할 공간 없으면 그 방향으로 회전하고 2번으로 즉 왼쪽으로 회전후에 왼쪽방향 탐색 -> 다시 회전
			// c. 네 방향 모두 청소가 이미 되있거나 벽인 경우, 바라보는 방향 유지한채로 한칸 후진하고 다시 왼쪽 탐색
			// d. 모두 청소되고 벽이면서 뒤쪽 방향이 벽이라 후진 못하면 작동을 멈춘다.
			boolean flag= false;
			for(int k=0;k<4;k++) {
				dir= (dir-1<0?3:dir-1); //음수이면 3으로 바꿔준다.
				int ny = y+dy[dir]; int nx=x+dx[dir];
				//방문하지 않았고, 벽이 아니고, 범위내 이면 이동한 것을 다시 큐에 담아준다.
