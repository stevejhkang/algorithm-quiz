...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		loc[y][x] = 1; //상어있는지를 저장
		svector2.push_back(make_pair(y, x));
	}
	int time = 0;
	int sum = 0;
	while (time < c) {
		...
		for (int i = 0; i < r; i++) {//가장 가까운 상어를 찾는다.
			if (loc[i][time] != 0) { //해당 위치에 상어가 있으면
				sum += v[i][time].z; //해당 상어 크기를 더해주고
									 //해당 상어의 정보를 삭제한다.
				//svector2.erase(svector.find(make_pair(i, time)));
				int idx = 0;
				for (int j = 0; j < svector2.size(); j++) {
					if (svector2[j].first == i && svector2[j].second == time) {
						idx = j;
					}
				}
				svector2.erase(svector2.begin()+idx);
		...
		for (int j = 0; j < svector2.size();j++) {
			...
			if (loc_temp[ny][nx] != 0) {//이미 있으면 크기 비교
				...
				temp_shark.y = ny; temp_shark.x = nx; temp_shark.d = nd;
				temp_shark.s = s; temp_shark.z = z;
				svector_temp2.push_back(make_pair(ny, nx));
